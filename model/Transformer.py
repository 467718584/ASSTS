import torch
import torch.nn as nn
import math


class PositionalEncoding(nn.Module):
    def __init__(self, d_model, max_len=5000):
        super(PositionalEncoding, self).__init__()
        pe = torch.zeros(max_len, d_model)
        position = torch.arange(0, max_len, dtype=torch.float).unsqueeze(1)
        div_term = torch.exp(torch.arange(0, d_model, 2).float() * (-math.log(10000.0) / d_model))
        pe[:, 0::2] = torch.sin(position * div_term)
        pe[:, 1::2] = torch.cos(position * div_term)
        pe = pe.unsqueeze(0)  # (1, max_len, d_model)
        self.register_buffer('pe', pe)

    def forward(self, x):
        # x: (batch, seq_len, d_model)
        x = x + self.pe[:, :x.size(1), :]
        return x


class TimeSeriesTransformer(nn.Module):
    def __init__(self, input_size=11, d_model=64, nhead=4, num_layers=2, dropout=0.1):
        super(TimeSeriesTransformer, self).__init__()

        # 1. 特征映射: 把输入维度(11)映射到 Transformer 维度(64)
        self.input_embedding = nn.Linear(input_size, d_model)
        self.pos_encoder = PositionalEncoding(d_model)

        # 2. Transformer Encoder 层
        encoder_layer = nn.TransformerEncoderLayer(d_model=d_model, nhead=nhead, batch_first=True, dropout=dropout)
        self.transformer_encoder = nn.TransformerEncoder(encoder_layer, num_layers=num_layers)

        # 3. 输出层
        # 使用 Global Average Pooling
        self.classifier = nn.Sequential(
            nn.Linear(d_model, 32),
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(32, 1),
            # nn.Sigmoid()  # 如果你的标签是 0-1 分数，保留 Sigmoid
        )

    def forward(self, x):
        # x shape: (batch_size, seq_len, input_size)

        # 映射 + 位置编码
        x = self.input_embedding(x)  # -> (batch, 120, 64)
        x = self.pos_encoder(x)

        # Transformer 计算
        x = self.transformer_encoder(x)  # -> (batch, 120, 64)

        # 聚合所有时间步的信息 (取平均)
        # 也可以只取最后一个时间步: x = x[:, -1, :]
        x = x.mean(dim=1)  # -> (batch, 64)

        output = self.classifier(x)
        return output